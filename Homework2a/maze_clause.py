...
class MazeClause:
    ...
    def __init__(self, props):
        ...
        Processes list of propositions to make a correctly formatted MazeClause
        """
        self.props = {}
        self.valid = False
        for p in props:
            if p[0] in self.props and self.props[p[0]] != p[1]:
                self.valid = True
                self.props = {}
                break
            self.props[p[0]] = p[1]
    ...
    def resolve(c1, c2):
        ...
        inference engine)
        """
        def not_resolved(p, combined):
            result = set()
            for x in combined:
                if x[0] != p:
                    result.add(x)
            return result
        # for every prop in c1 we need to check if it's in both clauses and check its value
        for p in c1.props:
            c1prop = c1.get_prop(p)
            c2prop = c2.get_prop(p)

            if c2prop is not None and c1prop != c2prop:
                combined = list(c1.props.items()) + list(c2.props.items())
                newClause = MazeClause(not_resolved(p, combined))
